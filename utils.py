...
def update_lr(lrate, lrate_decay, optimizer, step):
    decay_rate = 0.1
...
# Save Components
def save_component(basedir, expname, step, model, optimizer):
    save_folder = os.path.join(basedir, expname)
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    
    # step save
    np.save(os.path.join(save_folder,'step'), step)
    
    #model_save
    for key, val in model.items():
        save_model_name = os.path.join(save_folder,"{}_{:06d}.pth".format(key, step))
        torch.save(val.module.state_dict(), save_model_name)
        torch.save(val.module.state_dict(), os.path.join(save_folder,"{}_recent.pth".format(key)))

    #optimizer save
    save_optim_name = os.path.join(save_folder, "{}.pth".format("adam"))
    torch.save(optimizer.state_dict(), save_optim_name)
    
    print('Saved checkpoints at', save_folder)


# Save Video
def save_video(render_poses, hwf, chunk, render_kwargs_test, basedir, expname, step):
    with torch.no_grad():
        rgbs, disps = render_path(render_poses, hwf, chunk, render_kwargs_test)
...
def test_iter(images, poses, i_test, hwf, chunk, render_kwargs_test, basedir, expname, step):
    testsavedir = os.path.join(basedir, expname, 'testset_{:06d}'.format(step))
    os.makedirs(testsavedir, exist_ok=True)
    print('test poses shape', poses[i_test].shape)
    with torch.no_grad():
        render_path(poses[i_test], hwf, chunk, render_kwargs_test, gt_imgs=images[i_test], savedir=testsavedir)
